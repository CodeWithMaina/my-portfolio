...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const scrollTl = gsap.timeline({
        scrollTrigger: {
          ...
          onLeaveBack: () => {
            ...
            gsap.to(underlineRef.current, { x: 0, opacity: 1, scaleX: 1, duration: 0.3, overwrite: true });
            gsap.to(portraitRef.current, { x: 0, y: 0, rotation: 0, opacity: 1, scale: 1, duration: 0.3, overwrite: true });
            gsap.to(captionRef.current, { opacity: 1, duration: 0.3, overwrite: true });
          },
        },
      });
      ...
      scrollTl.fromTo(
        ...
        { x: '15vw', y: '-5vh', rotation: 3, opacity: 0, scale: 0.95, ease: 'power2.in' },
        0.6
      );
  ...
  return (
    ...
    >
      <div ref={contentRef} className="relative h-full px-5 sm:px-6 lg:px-[7vw] pt-20 sm:pt-24 lg:pt-0">
        <div className="flex flex-col lg:flex-row items-center justify-center h-full gap-8 lg:gap-0">
          ...
          <div className="flex flex-col justify-center max-w-2xl w-full lg:w-auto text-center lg:text-left">
            ...
            >
              I build calm, high-performance systems—clear architecture, clean UI, and
              measurable impact.
            </p>

            {/* CTA Buttons */}
            <div ref={ctaRef} className="flex flex-wrap justify-center lg:justify-start gap-3 mt-6 lg:mt-10">
              <Button
